...
//tags that arent able to be clicked and dragged
...
var Drag = {
	...
	onMouseDown : function(event) {
		...
		{
			targetTag = event.srcElement.tagName;
			elem = event.srcElement;
		}
		
		//$('utags').innerHTML = targetTag + " " + elem.parentNode.parentNode.parentNode.tagName;
		//sort of a hack to make sure we are in the block header...
		if (!(elem.parentNode.className == 'blockhdr' ||
			  elem.parentNode.parentNode.parentNode.parentNode.className == 'blockhdr' ||
			  elem.parentNode.parentNode.parentNode.parentNode.parentNode.className == 'blockhdr'))
			return true;
		//TFZ added this to not allow dragging when clicking on certain elements
		if (MochiKit.Base.findValue(NO_DRAG_TAGS, targetTag) != -1)
			return true;
